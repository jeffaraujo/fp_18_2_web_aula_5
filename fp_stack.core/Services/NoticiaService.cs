...
namespace fp_stack.core.Services
{
    public class NoticiaService
    {
        ...
        //Injeção de dependencia para o Cache
        ...
        public IEnumerable<Noticia> GetItens(int total)
        {
            ...
            if (!_memoryCache.TryGetValue(key, out retorno)) //Verificando se tem cache, se não tiver, ele realiza o processo
            {
                Task.Delay(200).Wait();
                ...
                var cacheEntrieOptions = new MemoryCacheEntryOptions()
                    .SetAbsoluteExpiration(DateTime.Now.AddMinutes(30)); //Configurando o cache (Tempo de expiração)

                _memoryCache.Set(key, retorno, cacheEntrieOptions); //Incluíndo o cache
            }
